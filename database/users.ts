...
export class users extends os {
    ...
    async findUserId(user:User):Promise<any> {
        let result = await this.knex('users').select('*').where({'id': user.id});
        if(result.length == 0) return null;
        return result;
    }
    async findUserLogin(user:User):Promise<any> {
        let result = await this.knex('users').select('*').where({'name': user.name});
        if(result.length == 0) return null;
        return result;
    }
